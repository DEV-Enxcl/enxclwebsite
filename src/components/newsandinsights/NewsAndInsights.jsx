...
const NewsAndInsights = ({ title, sectitle }) => {
  ...
  const chunkedNewsItems = newsItems.reduce((resultArray, item, index) => {
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % chunkedNewsItems.length);
  };

  const handlePrev = () => {
    setCurrentIndex((prevIndex) => (prevIndex - 1 + chunkedNewsItems.length) % chunkedNewsItems.length);
  };
  ...
  return (
    <div className="md:px-24 px-8 py-8 flex flex-col gap-8">
      <div className="">
        {title &&
          <h6 className="text-base font-bold flex gap-2 items-center">
            <News /> {title}
          </h6>
        }
        <span className="text-[44px] font-bold">{sectitle}</span>
      </div>
      ...
      <div className="hidden md:block">
        <Carousel>
          <CarouselContent>
            {chunkedNewsItems.map((newsGroup, groupIndex) => (
              <CarouselItem key={groupIndex}>
                <div className="flex justify-center gap-10">
                  {newsGroup.map((news, index) => (
                    <Card key={index} className="w-[350px] rounded-2xl hover:bg-[#ffb451] cursor-pointer">
      ...
      <div className='md:hidden flex flex-col'>
        ...
        <div className="flex justify-between flex-wrap gap-3 md:gap-0 items-center">
          {chunkedNewsItems[currentIndex].map((item, index) => (
            <Card
              key={index}
              className="md:w-[350px] rounded-2xl hover:bg-[#FFCE714D] cursor-pointer w-full"
            >
              <CardHeader className="p-0">
                <img
                  src={item.image}
              ...
              <CardContent>
                <div className="grid w-full">
                  <span className="font-normal p-2 mt-2 text-justify break-words hyphens-auto pt-5 text-black">
                    {item.description}
                  </span>
                </div>
                <Dialog>
                  ...
                  <DialogContent className="md:min-w-[900px] max-h-[700px] overflow-auto">
                    <DialogHeader>

                      <DialogDescription>
                        <div className='flex gap-10 md:flex-row flex-col-reverse md:max-h-[400px] max-h-max overflow-auto'>
                          <div className='w-full'>
                            <div>
                              <span className='text-[20px] font-semibold text-black'>{item?.heading}</span>
                            </div>
                            <p className='text-black text-justify break-words hyphens-auto mt-3'>
                              {item.description2}
                            </p>
        ...
        <div className="flex gap-3 justify-center pt-5">
          <button
            onClick={handlePrev}
            className="border border-black p-4 rounded-full hover:bg-[#ffbe47]"
          >
            <LeftArrow />
          </button>
          <button
